...
export function estimatePayoutTime(currentXP: number, avgXPGainPerLoop: number, loopIntervalSeconds = 30) {
  const { next } = getCurrentTier(currentXP);

  if (!next) return { etaMinutes: 0, etaText: 'Max crate unlocked' };
  ...
  return { etaMinutes, etaText, nextTier: next.name };
}
